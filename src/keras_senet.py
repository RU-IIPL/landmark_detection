...
class SEResNeXt(object):
    def __init__(self, inputs, num_outputs, depth=64, reduction_ratio=4, num_split=8, num_block=3, mtl=False):
        ...
        if mtl:
            self.model = self.build_model_mtl(Input(shape=inputs), num_outputs)
        else:
            self.model = self.build_model(Input(shape=inputs), num_outputs)
    ...
    def build_model(self, inputs, num_outputs):
        '''
        Build a SENet model.
        '''
        ...
        dense = Dense(units=num_outputs)(x)
        model = Model(inputs=inputs, outputs=dense)

        return model

    def build_model_mtl(self, inputs, num_outputs):
        '''
        Build a SENet model.
        '''
        x = self.initial_layer(inputs)
        ...
        x = GlobalAveragePooling2D()(x)
        #x = Dense(units=num_classes, activation='softmax')(x)
        x_main = Dense(units=num_outputs, name='main')(x)
        x_sub1 = Dense(2, name='sub1', activation='softmax')(x)
        x_sub2 = Dense(5, name='sub2', activation='softmax')(x)
        x_sub3 = Dense(42, name='sub3')(x) #3dpoint
        #model = Model(inputs=input, outputs=[x_main, x_sub1, x_sub2])
        model = Model(inputs=inputs, outputs=[x_main, x_sub1, x_sub2, x_sub3])
